  ...
  return (
    <div className="max-w-5xl space-y-6">
      <div>
        ...
        <p className="text-sm theme-text-secondary mt-1">Upload product images for AI grading & routing via Amazon Rekognition</p>
      </div>

      <div className="flex items-center gap-3 bg-white dark:bg-slate-900 border theme-border rounded-xl p-4">
        {['Upload', 'AI Processing', 'Result'].map((label, i) => {
          const idx = step === 'upload' ? 0 : step === 'processing' ? 1 : 2
          return (
            <React.Fragment key={label}>
              <div className="flex items-center gap-2 flex-1">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${i <= idx ? 'bg-amber-500 text-slate-950' : 'bg-slate-800 theme-text-muted'}`}>
                  {i < idx ? <CheckCircle2 size={16}/> : i + 1}
                </div>
                <span className={`text-xs font-medium ${i <= idx ? 'text-amber-400' : 'theme-text-muted'}`}>{label}</span>
              </div>
              {i < 2 && <ArrowRight size={14} className="text-slate-700"/>}
            </React.Fragment>
          )
        })}
      </div>
      ...
      {step === 'upload' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="space-y-4">
            <label className="block border-2 border-dashed theme-border hover:border-amber-500/50 rounded-xl p-8 text-center cursor-pointer transition bg-slate-900">
              ...
              <div className="grid grid-cols-2 gap-3">
                {images.map((src, i) => (
                  <div key={i} className="relative aspect-square rounded-lg overflow-hidden border theme-border">
                    ...
                    <button onClick={() => setImages(p => p.filter((_, x) => x !== i))} className="absolute top-1 right-1 w-5 h-5 bg-red-500 rounded-full theme-text text-xs flex items-center justify-center">x</button>
                  </div>
                ))}
              </div>
            )}
          </div>
          <div className="bg-white dark:bg-slate-900 border theme-border rounded-xl p-6 space-y-5">
            <h3 className="text-sm font-bold theme-text uppercase tracking-wider">Item Details</h3>
            <div>
              <label className="block text-xs font-semibold theme-text-secondary uppercase mb-2">Original Price</label>
            ...
            <button onClick={processReturn} disabled={images.length === 0 || !price} className="w-full bg-gradient-to-r from-amber-500 to-orange-500 hover:from-emerald-600 hover:to-teal-600 disabled:from-slate-700 disabled:to-slate-700 text-slate-950 font-bold py-3 rounded-xl transition flex items-center justify-center gap-2 text-sm">
      ...
      {step === 'result' && result && (
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white dark:bg-slate-900 border theme-border rounded-xl p-5 text-center">
              <p className="text-[10px] theme-text-muted uppercase tracking-widest font-semibold">Grade</p>
              <p className={`text-2xl font-black mt-2 ${result.grade==='Grade A'?'text-amber-400':result.grade==='Grade B'?'text-blue-400':result.grade==='Grade C'?'text-amber-400':'text-red-400'}`}>{result.grade}</p>
            </div>
            <div className="bg-white dark:bg-slate-900 border theme-border rounded-xl p-5 text-center">
              <p className="text-[10px] theme-text-muted uppercase tracking-widest font-semibold">Routing</p>
              <p className="text-2xl font-black mt-2 text-teal-400">{result.routing}</p>
            </div>
            <div className="bg-white dark:bg-slate-900 border theme-border rounded-xl p-5 text-center">
              ...
              <div className="w-full bg-slate-800 rounded-full h-2 mt-3"><div className={`h-2 rounded-full ${result.damageScore<20?'bg-amber-500':result.damageScore<40?'bg-blue-500':result.damageScore<60?'bg-amber-500':'bg-red-500'}`} style={{width:`${result.damageScore}%`}}/></div>
            </div>
          </div>
